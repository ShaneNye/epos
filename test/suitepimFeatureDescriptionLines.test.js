...
test("60 Night Comfort Trial uses reason 2709 for mattress items", () => {
  [browserSource, reasonsSource].forEach((source) => {
    assert.match(source, /classInternalId === "15" \|\| className === "mattress"/);
  ...
  assert.match(browserSource, /state\.options\.get\("reasons to buy"\)/);
  assert.match(reasonsSource, /reasonLookup\.byId\.get\("2709"\)/);
...
test("description generator pages use the current cache-busted assets", () => {
  assert.match(managementHtml, /suitepimWebManagement\.js\?v=description-template-4/);
  assert.match(reasonsHtml, /suitepimReasonsToBuy\.js\?v=description-template-4/);
});
