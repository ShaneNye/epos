(function () {
  ...
  function webDescriptionHtml(row) {
    ...
    const productInfoHtml = `<div style="display:grid; gap:7px; max-width:420px;">${detailItems.map(([label, value]) => `<div style="display:grid; grid-template-columns:110px minmax(0, 1fr); gap:10px; align-items:center; padding:8px 10px; background:#f8fafc; border-left:3px solid #0b7aa6; font-size:13px;"><strong style="font-size:12px; color:#4b5563; text-transform:uppercase; letter-spacing:0.02em;">${escapeHtml(label)}</strong><span style="font-weight:700; color:#0f172a;">${value}</span></div>`).join("")}</div>`;
